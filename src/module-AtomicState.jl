...
function tryRun(rep::AtomicState.Representation; nSample::Int64=2000, printout::Bool=true)
    ...
    if  printout
        ...
        println(">>")
        println(">>  1. CAP THE HEAP:   julia --heap-size-hint=8G --project=. yourjob.jl")
        println(">>     A JAC EOL solve ALLOCATES far more than it holds -- measured 14-Sep-2026, 133.5 GB allocated")
        println(">>     over a 130 s solve whose live data is about 2 GB, with 12 % of the wall clock already in garbage")
        println(">>     collection.  What a running job occupies is mostly heap grown to absorb that churn, and Julia")
        println(">>     does not hand it back.  The hint caps the heap and collects harder instead of growing:  it trades")
        println(">>     TIME for MEMORY, needs no code change, and is set per job.  It is the first thing to try.")
        println(">>")
