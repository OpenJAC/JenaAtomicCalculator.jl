...
`module  JAC.Hfs`  
    ... a submodel of JAC that contains all methods for computing HFS A, B and C coefficients, hyperfine-level representations, etc.
        Hyperfine coupling is expressed throughout as F = I + J, i.e. the nucleus is coupled to a finished electronic ASF (Level) rather
        than to individual CSF. Working with ASF keeps the number of hyperfine levels moderate and makes the results directly interpretable;
        the computational effort is formally equivalent.
        ...
        Build one with Hfs.computeHyperfineMultiplet(multiplet, nm, grid, settings), which chains Hfs.defineHyperfineBasis and
...
`Hfs.amplitude(mp::EmMultipole, rLevel::Level, sLevel::Level, grid::Radial.Grid; printout::Bool=true)`
    ... to compute the T^(M1), T^(E2) or T^(M3) hyperfine amplitude for a given pair of levels. A value::ComplexF64 is returned.

        CONVENTION -- read this before using the result in a recoupling formula (clarified 05-Aug-2026). What is returned is the
        "GRASP-like" amplitude, i.e. <alpha_r J_r || T^(mp) || alpha_s J_s> / sqrt(2J_r+1), and NOT the reduced matrix element in the
        standard Wigner-Eckart normalization. That is exactly what the A, B and C constants formed in computeAmplitudesProperties expect --
        A*I/mu = amplitude/sqrt(J(J+1)), verified against the measured A(H 1s) to 0.06% -- but it is NOT what a Wigner-6j expression
        ...
        InteractionStrength.hfs_tM1. Fixed by dividing each term by sqrt(2*j_a+1), undoing the internal GRASP-like conversion, matching
        IsotopeShift's convention.
"""
function  amplitude(mp::EmMultipole, rLevel::Level, sLevel::Level, grid::Radial.Grid; printout::Bool=true)
    # E3 ADDED 06-Aug-2026, and with it the operator-dependent parity rule below. InteractionStrength.hfs_tE3 had existed all along
    # (Andersson & Jonsson 2008, Eq. 49) but was never dispatched here, because E3 is the first PARITY-ODD hyperfine multipole: M1, E2 and
    # M3 all carry parity +1 -- (-1)^L for electric, (-1)^(L+1) for magnetic -- whereas E3 carries -1. The old guard `rLevel.parity !=
    # sLevel.parity -> 0` is correct for the even ones and silently wrong for E3, which requires the two electronic levels to have OPPOSITE
    # parity.
    ...
    if        mp == M1    opa = SpinAngular.OneParticleOperator(1, plus);  hfsFunc = InteractionStrength.hfs_tM1
    elseif    mp == E2    opa = SpinAngular.OneParticleOperator(2, plus);  hfsFunc = InteractionStrength.hfs_tE2
    elseif    mp == M3    opa = SpinAngular.OneParticleOperator(3, plus);  hfsFunc = InteractionStrength.hfs_tM3
    elseif    mp == E3    opa = SpinAngular.OneParticleOperator(3, minus);  hfsFunc = InteractionStrength.hfs_tE3
...
`Hfs.computeAmplitudesProperties(outcome::Hfs.Outcome, nm::Nuclear.Model, grid::Radial.Grid, settings::Hfs.Settings, 
    ...
    ... to compute all amplitudes and properties of for a given level; an outcome::Hfs.Outcome is returned for which the amplitudes and
        properties are evaluated explicitly.
"""
...
`Hfs.computeHyperfineMultiplet(level::Level, nm::Nuclear.Model, grid::Radial.Grid)`  
    ... to compute a hyperfine multiplet, i.e. a representation of hyperfine levels within a hyperfine-coupled basis as defined by the given
        (electronic) level; a hfMultiplet::hfMultiplet is returned.
"""
...
`Hfs.computeHyperfineMultiplet(multiplet::Multiplet, nm::Nuclear.Model, grid::Radial.Grid, settings::Hfs.Settings; output=true)`  
    ... to compute a hyperfine multiplet, i.e. a representation of hyperfine levels within a hyperfine-coupled basis as defined by the given
        (electronic) multiplet; an hfMultiplet::Hfs.HfMultiplet is returned.
"""
...
`Hfs.computeHyperfineRepresentation(hfBasisVectors::Array{HfBasisVector,1}, nm::Nuclear.Model, grid::Radial.Grid)`
    ... to set-up and diagonalize the Hamiltonian matrix of H^(DFB) + H^(hfs) within the atomic hyperfine (IJF-coupled) basis; a
        hfMultiplet::Hfs.HfMultiplet is returned.

        REPAIRED 05-Aug-2026. This function had never run: its body addressed an undefined `hfBasis.vectors[...]` instead of its own
        argument, it called Hfs.HfLevel(...) with seven arguments against a six-field struct, and it carried `* 1.0e-6 # fudge-factor to
        keep HFS interaction small` on BOTH the magnetic and the electric contribution -- a debugging leftover that would have scaled every
        hyperfine splitting down by a million. The fudge factor is removed: the splitting of the initial ion is precisely the observable
        this representation exists to produce. Nothing can regress, since the function could not previously be called.

        The matrix is block diagonal in F and parity by construction (the guards below), so the eigenvectors do not mix different F; each
        hyperfine level is Sum_k mc[k] |(I J_k) F>, where the Clebsch-Gordan coupling already sits inside each HfBasisVector and mc
        describes the J-mixing among vectors of equal F.

        With mu = Q = 0 the hyperfine Hamiltonian vanishes identically and only the electronic energies remain on the diagonal. Since those
        are non-degenerate, the eigenvectors come out as pure unit vectors, i.e. mc = [.. 1.0 ..] on a single basis vector. The "no
        hyperfine mixing" case therefore needs no separate code path -- it is the exact output of this same routine.
"""
function computeHyperfineRepresentation(hfBasisVectors::Array{HfBasisVector,1}, grid::Radial.Grid;
    ...
    for  r = 1:n
        for  s = 1:n
            ...
            matrix[r,s] = 0.
            # THE DIAGONAL carries the electronic energy AND the nuclear excitation energy of its isomer. The latter was missing before
            # 06-Aug-2026, which did not matter while a basis held one nuclear state, and is fatal the moment it holds two: without it the
            # ground and isomeric levels are degenerate, and the nuclear hyperfine mixing -- which is of order V_21 / dE_nuc -- is
            # ill-defined.
            if  r == s
            ...
            # The TOTAL parity, nuclear x electronic, must match; with one nuclear state the nuclear factor is common and the electronic
            # test suffices, with two it does not.
            ...
            # The hyperfine operator is H_hfs = sum_k T^k(electronic) . W^k(nuclear), a scalar product of two tensors acting on the two
            # parts of the coupled system, so

            #   <(I_r J_r) F| H_hfs |(I_s J_s) F> = sum_k (-1)^(I_r+J_s+F) {F J_r I_r; k I_s J_s}
            #                                              <I_r||W^k||I_s> <J_r||T^k||J_s>

            # For I_r = I_s = I and J_r = J_s = J this reduces to the Casimir form against which this matrix was verified on 05-Aug-2026.
            # The OFF-DIAGONAL-in-isomer case is new (06-Aug-2026) and is what nuclear hyperfine mixing consists of: nothing changes but
            # that <I_r||W^k||I_s> now connects two DIFFERENT nuclear states. Hermiticity of the result is checked below, which is what
            # would catch a wrong phase.
            for  mp in hfMultipoles
                ...
                # UNITS. computeInteractionAmplitudeM returns the DIAGONAL nuclear element from mu [nuclear magnetons] or Q [barn], which
                # must be converted; but for two DIFFERENT isomers it returns elementM, which the Isomer docstring already defines to be in
                # atomic units. Converting uniformly would therefore be wrong by ~2.7e-4 or ~3.6e-2 on precisely the off-diagonal elements
    ...
    # Hermiticity: the matrix is built term by term for both (r,s) and (s,r), so a wrong phase convention in the off-diagonal-in-isomer
    # element shows up here as a SIGN difference rather than a small rounding one.
    asym = 0.;   scale = 0.
    for  r = 1:n, s = 1:n
    ...
    for  ev = 1:length(eigen.values)
        # Construct the eigenvector with regard to the given basis (not w.r.t the symmetry block). F and parity are read off the DOMINANT
        # component rather than the first one above a threshold: with J-mixing the first component need not be the leading one, and a
        # mis-assigned F would silently corrupt every recoupling coefficient computed from this level later on.
        ...
        # M = F, not 0: a projection must share the integer/half-integer character of its F, and 0 is simply not a valid projection of a
        # half-integer F. M is otherwise only a label here -- in a free ion the 2F+1 sublevels are exactly degenerate, and every quantity
        # computed from these levels (energies, rates, strengths) is summed over final and averaged over initial M, so M cancels by the
        # Wigner-Eckart theorem and only reduced matrix elements survive. It becomes meaningful only once something breaks the isotropy --
        # an external field, an aligned or polarized ensemble, or an angular distribution.
        MF        = AngularM64(F)
        newlevelF = Hfs.HfLevel(F, MF, parity, 0, en, hfBasisVectors, evector)
        push!( levelFs, newlevelF)
    end
    # A STABLE INDEX, assigned in order of increasing energy (added 06-Aug-2026). Hyperfine levels previously carried none, so nothing could
    # refer to one: Basics.selectLevelPair dispatches on ManyElectron.Level and throws a MethodError on an HfLevel, and Hfs-based display
    # routines had to key on the surrogate (2F, energy) pair instead. Energy order is a safe labelling here because hyperfine multiplets of
    # neighbouring electronic levels essentially never interleave -- a hyperfine splitting is orders of magnitude smaller than a
    # fine-structure one.
...
`Hfs.computeInteractionAmplitudeM(mp::EmMultipole, leftIsomer::Nuclear.Isomer, rightIsomer::Nuclear.Isomer)`
    ... to compute the hyperfine interaction amplitude (<leftIsomer || M^(mp)) || rightIsomer>) for the interaction of two nuclear levels;
        this ME is geometrically fixed if the left and right isomer are the same, and it depends on the nuclear ME otherwise. An
...
function  computeInteractionAmplitudeM(mp::EmMultipole, leftIsomer::Nuclear.Isomer, rightIsomer::Nuclear.Isomer)
    ...
    if  leftIsomer == rightIsomer
        ...
        elseif   mp == E2       amplitude = leftIsomer.Q / 2 * sqrt( (floatI + 1)*(2*floatI+1) * (2*floatI + 3)/ (floatI * (2*floatI -1)) )
            ...
            # DIAGONAL higher multipoles are set to zero rather than raising an error (06-Aug-2026). This branch is the STATIC hyperfine
            # moment of one nuclear state, which Nuclear.Isomer supplies only for M1 (mu) and E2 (Q); no octupole moment is part of its
            # input, and the meaning of the `Omega` field is not documented well enough to be used here silently. The omitted term is a
            # small static shift of a level, whereas the OFF-DIAGONAL element of the same multipole -- the one that mixes two nuclear states
            # and drives every rate in HyperfineInduced -- is taken from `elementM` below and is fully included. Raising an error instead
            # simply made every E3 case unrunnable, which is how the 235U branch was blocked.
            amplitude = 0.
        end
    else
...
`Hfs.computeInteractionMatrix(basis::Basis, grid::Radial.Grid, settings::Hfs.Settings)`
    ... to compute the T^M1 and/or T^E2 interaction matrices for the given basis, i.e. (<csf_r || T^(n)) || csf_s>). An
        ...
        for  r = 1:ncsf
            for  s = 1:ncsf
                if  basis.csfs[r].parity  != basis.csfs[s].parity   continue    end 
                 # Calculate the spin-angular coefficients
                subshellList = basis.subshells
                opa = SpinAngular.OneParticleOperator(2, plus)
                wa  = SpinAngular.computeCoefficients(opa, basis.csfs[r], basis.csfs[s], subshellList) 
        ...
        for  r = 1:ncsf
            for  s = 1:ncsf
                if  basis.csfs[r].parity  != basis.csfs[s].parity   continue    end 
                 # Calculate the spin-angular coefficients
                subshellList = basis.subshells
                opa = SpinAngular.OneParticleOperator(3, plus)
                wa  = SpinAngular.computeCoefficients(opa, basis.csfs[r], basis.csfs[s], subshellList) 
    ...
    for  upperLevF in  upperOutcome.hfsMultiplet.hfLevels
        for  lowerLevF in  lowerOutcome.hfsMultiplet.hfLevels
            for  (m, mp)  in  enumerate(multipoles)
                ...
                # sa = sa * @sprintf("%.6e", Basics.recast("rate: radiative, to Einstein A",  line, chRate)) * "  " sa = sa *
                # @sprintf("%.6e", Basics.recast("rate: radiative, to Einstein B",  line, chRate)) * "    "
...
`Hfs.computeOutcomes(multiplet::Multiplet, nm::Nuclear.Model, grid::Radial.Grid, settings::Hfs.Settings; output=true)`  
    ... to compute (as selected) the HFS A, B and C parameters as well as hyperfine energy splittings for the levels of the given multiplet
        ...
        with all the results are returned.
"""
...
`Hfs.defineHyperfineBasis(multiplet::Multiplet, isomers::Array{Nuclear.Isomer,1}; printout::Bool=true)`
    ... to define/set-up an atomic hyperfine (IJF-coupled) basis for the given electronic multiplet and ONE OR MORE nuclear states; an
        Array{Hfs.HfBasisVector,1} is returned, one vector for every (isomer, electronic level, F) triple allowed by F = I + J.

        Admitting more than one isomer (06-Aug-2026) is what makes nuclear hyperfine mixing expressible: two nuclear levels of the same F,
        built on the same electronic level, then appear as two basis vectors, and diagonalizing the hyperfine matrix over that basis mixes
        them. Nothing else in the machinery changes. The parity stored on each vector is the TOTAL one, nuclear x electronic.

        REPAIRED 05-Aug-2026; see the note on the single-level method above. The nuclear moments are carried into each basis vector through
        a Nuclear.Isomer built from the given model, so that a model with mu = Q = 0 produces a basis whose hyperfine Hamiltonian vanishes
        identically -- the device used to switch the hyperfine structure of intermediate and final levels off without a separate code path.
"""
...
`Hfs.defineHyperfineBasis(multiplet::Multiplet, nm::Nuclear.Model; printout::Bool=true)`
    ... the single-isomer method; an Array{Hfs.HfBasisVector,1} is returned. It simply assembles one Nuclear.Isomer from the given nuclear
        model and defers to the method above, so that the two cannot diverge.
"""
function  defineHyperfineBasis(multiplet::Multiplet, nm::Nuclear.Model; printout::Bool=true)
    ...
    # The nuclear parity is not carried by Nuclear.Model; it is taken as even, so that the total parity of a basis vector reduces to the
    # electronic one.
...
`Hfs.determineOutcomes(multiplet::Multiplet, settings::Hfs.Settings)`  
    ... to determine a list of Outcomes's for the computation of HFS A- and B-parameters for the given multiplet. It takes into account the
        particular selections and settings. An Array{Hfs.Outcome,1} is returned. Apart from the level specification, all physical properties
...
`Hfs.displayNondiagonal(stream::IO, multiplet::Multiplet, grid::Radial.Grid, settings::Hfs.Settings)`  
    ... to compute and display all non-diagonal hyperfine amplitudes for the selected levels. A small neat table of all (pairwise) hyperfine
        amplitudes is printed but nothing is returned otherwise.
"""
...
`Hfs.displayOutcomes(outcomes::Array{Hfs.Outcome,1})`  
    ... to display a list of levels that have been selected for the computations A small neat table of all selected levels and their
        energies is printed but nothing is returned otherwise.
"""
...
function  displayResults(stream::IO, outcomes::Array{Hfs.Outcome,1}, nm::Nuclear.Model, settings::Hfs.Settings)
    ## 2 + (10+2) + (10+4) + (14+10) + (54+8) = 114 after the g_J column was removed on 18-Aug-2026; it was 128.
    nx = 114
    println(stream, " ")
...
`Hfs.recouplingElectronicOperator(spinI::AngularJ64, Ja::AngularJ64, Fa::AngularJ64,
    ...
    ... to return the factor that turns an ELECTRONIC reduced matrix element of rank L into the corresponding one between IJF-coupled
        levels, for an operator that acts on the electrons alone and leaves the nucleus untouched:

            <(I J_b) F_b || T^L || (I J_a) F_a>
                = (-1)^(I+J_b+F_a+L) sqrt((2F_a+1)(2F_b+1)) {J_b F_b I; F_a J_a L} <J_b || T^L || J_a>

        A value::Float64 is returned. This is the standard result for a tensor acting on one part of a coupled system (Edmonds 7.1.7), with
        the nucleus as the spectator.

        VERIFIED (work/diag-recoupling.jl, 05-Aug-2026) over 454 combinations of I, J_a, J_b, F_a and L = 1, 2: the sum over F_b of the
        squared factor equals (2F_a+1)/(2J_a+1) to better than 1e-15, which is exactly what leaves a radiative width unchanged by hyperfine
        coupling. For I = 0 the factor reduces to 1.

        Lives here, in the property module, rather than in any one process module: both hyperfine-resolved dielectronic recombination and
        hyperfine-induced transitions need it, and a second copy is how two implementations of the same formula start to drift apart.
"""
...
`Hfs.recouplingNuclearOperator(J::AngularJ64, Ia::AngularJ64, Fa::AngularJ64,
    ...
    ... the mirror image of Hfs.recouplingElectronicOperator: the factor that turns a NUCLEAR reduced matrix element of rank L into the
        corresponding one between IJF-coupled levels, for an operator that acts on the nucleus alone and leaves the electrons untouched:

            <(I_b J) F_b || W^L || (I_a J) F_a>
                = (-1)^(J+I_b+F_a+L) sqrt((2F_a+1)(2F_b+1)) {I_b F_b J; F_a I_a L} <I_b || W^L || I_a>

        A value::Float64 is returned. It is the same expression with the roles of the nuclear and electronic angular momenta interchanged,
        because the coupled state is symmetric in the two: whichever subsystem the operator acts on, the other is the spectator that appears
        in the 6-j.

        This is the factor that carries a hyperfine-induced NUCLEAR transition -- the isomer changes while the electronic level does not --
        and it is therefore what the 229Th and 205Pb cases rest on. For J = 0 it reduces to 1.
"""
...
`Hfs.reducedMeElectronic(mp::EmMultipole, rLevel::Level, sLevel::Level, grid::Radial.Grid)`
    ... to return the electronic reduced matrix element <alpha_r J_r || T^(mp) || alpha_s J_s> in the STANDARD (Wigner-Eckart)
        normalization, i.e. the quantity that may be inserted directly into a Wigner-6j recoupling expression. A value::ComplexF64 is
        returned.

        WHY THIS EXISTS (05-Aug-2026). Hfs.amplitude does NOT return that quantity, despite what its own header line says: it returns the
        reduced element divided by sqrt(2J+1), the "GRASP-like" normalization. The two differ by a factor that is invisible in every place
        where the same convention is used on both sides, and fatal in any place that mixes them -- as Hfs.computeHyperfineRepresentation
        did, building a 6-j expression directly out of Hfs.amplitude and coming out too small by exactly sqrt(2J+1).

        HOW THIS WAS SETTLED, since the two candidate conventions cannot both be right: A(H 1s) computed in the BARE NUCLEAR potential --
        NuclearField(), not a DFS field, whose self-interaction on a one-electron system puts the 1s orbital at -0.194 instead of -0.5 a.u.
        and invalidates the benchmark entirely -- gives 1421.21 MHz against the measured 1420.406 MHz, a ratio of 1.00057, using
        computeAmplitudesProperties' formula A*I/mu = amplitude/sqrt(J(J+1)). Inserting a further sqrt(2J+1) would give 1004.9 MHz, i.e. 29%
        low. So the A/B/C constants are right as they stand and it is the recoupling side that needed the factor.

        Note that A*I/mu = amplitude/sqrt(J(J+1)) and A*I/mu = <J||T^1||J>/sqrt(J(J+1)(2J+1)) are the SAME statement once <J||T^1||J> =
        amplitude*sqrt(2J+1); computeAmplitudesProperties therefore needs no change, and none was made. This function only makes the
        convention explicit so that the next caller who needs a genuine reduced matrix element does not have to rediscover it.

        For J_r != J_s the factor is taken as the symmetric ((2J_r+1)(2J_s+1))^(1/4), which reduces to sqrt(2J+1) on the diagonal and keeps
        the hyperfine matrix symmetric. The diagonal is what the H(1s) benchmark pins down; the off-diagonal convention is not separately
        verified here, and matters only through J-mixing, which is of order 1e-5 whenever the hyperfine splitting is small against the fine
        structure.
"""
