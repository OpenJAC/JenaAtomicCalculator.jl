...
# FINE-STRUCTURE RESOLVED DIELECTRONIC RECOMBINATION
#
...
#
# What matters is WHERE the coupling between the two steps lives. The two total widths
#
...
# STORED on every CaptureLine, so that nothing downstream re-derives them by scanning a list -- which is where
# an aggregation error would otherwise hide.
#
...
#
# This file is a COMPOSITE: it has no channel type of its own, and its two line types are built out of the
# other modules' --
#
#     CaptureLine.capturePartialWaves ::Array{AutoIonization.PartialWave,1}
#     PhotonLine.photonAmplitudes     ::Array{MultipoleAmplitude,1}
#
# WHERE EmPropertyC BELONGS, AND WHERE IT DOES NOT. The photon AMPLITUDES are complex and gauge-paired, so they
# are EmPropertyC -- and they arrive already so, inside the embedded PhotoEmission type, rather than as a field
# declared here. The four observables of this file are RATES and STRENGTHS: captureRate and totalAugerRate are
# real and gauge-free (the Auger operator has no gauge), while totalPhotonRate and resonanceStrength are real
# and gauge-dependent. Float64 and EmProperty are therefore right for all four, and an EmPropertyC would be
# wrong -- they are not complex quantities.
#
# The gauge machinery itself is not duplicated here: determinePhotonChannels calls into PhotoEmission for the
# gauge loop and the hasMagnetic flag, and computePhotonAmplitudes reduces to one abs2 on an EmPropertyC.
#
# checkConsistentMultiplets, checkOrbitalRepresentation, determineCaptureChannels, determinePhotonChannels and
# isResonanceToBeExcluded are route-independent helpers; the last of them is used by
# module-Cascade-inc-simulations.jl.



"""
`struct  DielectronicRecombination.CaptureLine`
    ... represents one capture step  i + e^- --> m,  carrying the partial waves of the captured electron together
        with the two total widths of the intermediate level m and the resonance strength of the resonance.
...
`struct  DielectronicRecombination.PhotonLine`
    ... represents one stabilization step  m --> f + hv,  carrying one entry per MULTIPOLE, each of which holds
        both gauges.
...
`DielectronicRecombination.computeCaptureAmplitudes(captureLine::DielectronicRecombination.CaptureLine,
    ...
    ... computes the Auger amplitudes of the capture step on its partial waves; a CaptureLine with the amplitudes
        and the capture rate filled is returned. The two total widths and the resonance strength remain zero here.
"""
function  computeCaptureAmplitudes(captureLine::DielectronicRecombination.CaptureLine, nm::Nuclear.Model,
    ...
    for  pw in captureLine.capturePartialWaves
        ...
        # The computed phase, as AutoIonization's own driver does: AutoIonization.amplitude multiplies by
        # exp(-im*phase).
...
`DielectronicRecombination.computeHydrogenicPhotonRate(intermediateLevel::Level, electronEnergy::Float64,
    ...
    ... to estimate the radiative rate by which the captured Rydberg electron itself stabilizes the resonance, i.e.
        ...
        RE-AUTOIONIZATION CUTOFF. Not every such decay actually completes the recombination.
        ...
        THE SUMMATION RANGE. computeHydrogenicRate returns exactly 0.0 unless nf < n_R, so the sum runs DOWNWARD
        from the captured Rydberg shell,  nf = nFinal+1 : min(nHydrogenic, n_R - 1).  Two mistakes are easy here and
        either one makes the whole correction identically zero:

        (1) running  nf = nFinal+1 : nHydrogenic, i.e. UPWARD from the explicit final shells;
        (2) taking nFinal as the largest n in the final basis, which always contains the captured Rydberg shell
            itself, so that nFinal = n^(lowest-captured) and the range starts above n_R even in principle.
...
function  computeHydrogenicPhotonRate(intermediateLevel::Level, electronEnergy::Float64,
    ...
    # See the note on the summation range above.
    rate = 0.;   dropped = 0.
...
function  computeHydrogenicRate(ni::Int64, li::Int64, nf::Int64, lf::Int64,  Zeff::Float64)
    # Compute A(n,l) coefficient in the recursion formulas
...
`DielectronicRecombination.computePhotonAmplitudes(photonLine::DielectronicRecombination.PhotonLine,
                                                         grid::Radial.Grid)`
    ... computes the multipole amplitudes of the stabilization step  m --> f + hv;  a PhotonLine with the
        amplitudes and the radiative rate filled is returned.

        THIS IS WHERE EmPropertyC EARNS ITS PLACE. A three-way accumulation over the gauges --
              if  gauge == Coulomb  rateC += ...;  elseif Babushkin  rateB += ...;  elseif Magnetic  BOTH
        -- reduces to a single line, because abs2 of an EmPropertyC is an EmProperty and a magnetic amplitude has
        equal components and so enters both gauges by itself.

        The Einstein-A prefactor is  8pi * alpha * omega / (2J_m + 1),  with NO (2J_f+1) and NO 1/pi.
"""
...
`DielectronicRecombination.determineCaptureChannels(intermediateLevel::Level, initialLevel::Level,
    ...
    ... determines the partial waves of the electron captured in the step  i + e^- --> m;  an
...
`DielectronicRecombination.determineEmpiricalTreatment(finalMultiplet::Multiplet, intermediateMultiplet::Multiplet, 
    ...
    ... to determine an instance of empiricalTreatment::EmpiricalTreatment that is (internally) applied to simplify
        the use of "corrections" to the DR strengths. This data structure summarizes all parameters that help introduce
        ...
        A missing maximum_l resolves to 1000, i.e. no limit, so that MaximumlCorrection(missing) means what a user
        writing it intends; resolving it to 1 would silently suppress every Rydberg shell with l > 1.
"""
function determineEmpiricalTreatment(finalMultiplet::Multiplet, intermediateMultiplet::Multiplet, 
    ...
    #
    # n^(final) must NOT be taken as maximum(finalNs) over the whole final basis. A DR final level is |core> nl
    # with the SAME captured electron, so the Rydberg shell is always present in that basis and the maximum would
    # come out equal to n^(lowest-captured) -- 19 rather than the intended 3 for the Be-like gold case. That
    # contradicts the hierarchy this struct documents,
    #
    #      n^(core)  <  n^(final)  <  n^(hydrogenic)  <  n^(lowest-captured),
    #
    # and would make the correction range nFinal+1 : nHydrogenic start ABOVE the Rydberg shell, where the
    # hydrogenic rate is identically zero.  See also the note on computeHydrogenicPhotonRate.
...
`DielectronicRecombination.determinePhotonChannels(finalLevel::Level, intermediateLevel::Level,
                                                         settings::DielectronicRecombination.Settings)`
    ... determines the multipole components of the stabilization step  m --> f + hv;  an
        Array{MultipoleAmplitude,1} is returned with all amplitudes still zero.

        THIS IS A CALL, NOT A COPY. The selection rule is asked of the module that owns it, so that neither the
        gauge loop nor the hasMagnetic flag is repeated here. A wrapper exists at all only because
        PhotoEmission.determineChannels expects a PhotoEmission.Settings, so the multipoles have to be handed over.
"""
...
`DielectronicRecombination.isResonanceToBeExcluded(level::Level, refLevel, rSelection::ResonanceSelection)`  
    ...
    It returns false if the ResonanceSelection() is inactive or if level belongs to the selected resonances.
